...
var View = /** @class */ (function (_super) {
    ...
    function View(calendar, viewSpec) {
        ...
        console.log('<--Edit viewSpec-->', _this.viewSpec);
        // shortcuts
...
var EventRenderer = /** @class */ (function () {
    ...
    EventRenderer.prototype.render = function (eventsPayload) {
        ...
        if (this.view.type === 'month' && window['isMobile']) {
            setTimeout(function () {
                _this.setListCardEvent(fgRanges, $('td .fc-day.fc-today').data('date'));
            }, 100);
            var eventRenderer_1 = this;
            $('td .fc-day').click(function (ev) {
                ...
                eventRenderer_1.setListCardEvent(fgRanges, ev.target.dataset.date && ev.target.dataset.date.toString());
            });
        }
    ...
    EventRenderer.prototype.setListCardEvent = function (fgRanges, targetDate) {
        if (this.view.options.hasOwnProperty('isShowList') && !this.view.options.isShowList)
            return;
        var eventFootprints = this.component.eventRangesToEventFootprints(fgRanges);
        var segs = this.component.eventFootprintsToSegs(eventFootprints);
        var htmlBasicViewEventList = '';
        for (var fgRange in fgRanges) {
            // tslint:disable-next-line:radix
            var seg = segs[parseInt(fgRange)].footprint.getEventLegacy();
            var eventStartDate = seg.start;
        ...
        $('.eventBox').on('click', function (e) {
            // tslint:disable-next-line:radix
            var seg = segs[parseInt(e.target.id)];
            var modifySeg = seg.footprint.getEventLegacy();
...
var Calendar = /** @class */ (function () {
    ...
    Calendar.prototype.renderView = function (viewType, subViewType) {
    ...
    */
    ...
    Calendar.prototype.initEventManager = function () {
        ...
        if (window['isMobile']) {
            singleRawSource.forEach(function (element) {
                if (element.start !== element.end && element.end) {
                    console.log(element.end);
                    var startDay = moment(JSON.parse(JSON.stringify(element.start)));
                    var endDay = moment(JSON.parse(JSON.stringify(element.end)));
                    var diffDay = endDay.diff(startDay, 'days');
                    element.backupEnd = moment(JSON.parse(JSON.stringify(element.end)));
                    element.end = null;
                    for (var i = 1; i < diffDay + 1; i++) {
    ...
    ListEventRenderer.prototype.fgSegHtml = function (seg) {
        ...
        if (window['isMobile']) {
            htmlReturn = '<tr class="' + classes.join(' ') + '">' +
                ...
                '<span class="event-list-title">' + util_1.htmlEscape(eventDef.title || '') + '</span>' +
                '<span class="event-list-detail">' + util_1.htmlEscape(eventDef.description || '') + '</span>' +
                '</a>' +
                '</td>' + '<td>' + (timeHtml || '') + '</td>';
            '</tr>';
        }
